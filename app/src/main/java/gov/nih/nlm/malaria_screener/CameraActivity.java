...
import gov.nih.nlm.malaria_screener.findmarkers.MarkerBasedWatershed;
...
public class CameraActivity extends AppCompatActivity{
    ...
    int whichClassifier = 1; // 0 is DL, 1 is SVM
...
//        Log.d(TAG, "minEC: " + minEC);
        ...
        whichClassifier = Integer.valueOf(sharedPreferences.getString("classifier", "0"));
...
//                        }
        ...
        if (requestCode == REQUEST_GALLERY && resultCode == Activity.RESULT_OK) {
            ...
            Runnable r = new Runnable() {
                ...
                public void run() {
                    ...
                    Log.d(TAG, "resizedMat: " + resizedMat );

                    ProcessImage(oriSizeMat, resizedMat, RV);
                }
            };
    ...
    public void ProcessImage(Mat oriSizeMat, Mat resizedMat, float resizeValue) {
        ...
        MarkerBasedWatershed watershed = new MarkerBasedWatershed(resizedMat, resizeValue);
        ...
        if (reTakeFlag) { // take care of the case (avoid crash) when segmentation failed due to a plain black image was taken
            ...
            startTime_C = System.currentTimeMillis();

            Cells c = new Cells(watershed_result, oriSizeMat, context, WBC_Mask, whichClassifier);
            c = null;
            System.gc();
            Runtime.getRuntime().gc();
            ...
            if (whichClassifier == 1) {
                ...
                if (UtilsCustom.cellLocation.length == 0) {  //take care of the case(avoid crash) when segmentation passed but no cell chips extracted
                    ...
                    featureTable = UtilsCustom.featureTable; // this is from the new image, so the number of rows is number of cells
                    ...
                    for (int index = 0; index < classNum; index++) {
                        ...
                        ft_rows = featureTable.rows();
                        ft_cols = featureTable.cols();
                        runSVM(testT, index);

                        //OutputTextFile outputTextFile = new OutputTextFile(featureTable);
                        //Log.d(TAG, "featureRowNum: " + featureTable);
                    ...
                    Log.d(TAG, "totalTime_2: " + totalTime_2);

//                for (int j = 0; j < cellNum; j++) {
//                    Log.d(TAG, "all_dists[" + j + "]: " + all_dists[j][0] + " " + all_dists[j][1]);
//                }
                    ...
                    // get the first channel of the FeatureTable since all channels are the same
                    results = new double[UtilsCustom.FTchannels.get(0).rows()];
                    results = classLabels;

                    //draw results on image

                    for (int i = 0; i < featureTable.rows(); i++) {

                        Rect roi = new Rect(0, i, featureTable.cols(), 1);
                        Mat sample = new Mat();
                        sample = new Mat(UtilsCustom.FTchannels.get(0), roi);
                        sample.convertTo(sample, CvType.CV_32F);

                        if (results[i] == 1) {
                            //infectedNum++;
                            paint.setColor(Color.BLUE); // not infected
                            canvas.drawCircle(cellLocation[i][1] / RV, cellLocation[i][0] / RV, 2, paint);
                            ...
                            if (takenFromCam) { // test this canvas rotate
                                ...
                                if (orientation == Surface.ROTATION_0) { //portrait
                                    canvas.rotate(270, cellLocation[i][1] / RV, cellLocation[i][0] / RV);
                                } else if (orientation == Surface.ROTATION_270) { //landscape
                                    canvas.rotate(180, cellLocation[i][1] / RV, cellLocation[i][0] / RV);
                                } else if (orientation == Surface.ROTATION_180) { //reverse portrait
                                    canvas.rotate(90, cellLocation[i][1] / RV, cellLocation[i][0] / RV);
                                } else if (orientation == Surface.ROTATION_90) { // reverse landscape
                                    canvas.rotate(0, cellLocation[i][1] / RV, cellLocation[i][0] / RV);
                                }
                                canvas.drawText(String.valueOf(infectedCurrent), cellLocation[i][1] / RV - 7, cellLocation[i][0] / RV - 7, paint);
                                canvas.restore();
                            } else {
                                canvas.drawText(String.valueOf(infectedCurrent), cellLocation[i][1] / RV - 7, cellLocation[i][0] / RV - 7, paint);
                            }
                    ...
                    Log.d(TAG, "processingTime: " + processingTime);

                    goToNextActivity();

                }
            } else if (whichClassifier==0){

                Log.d(TAG, "results_NN: " + UtilsCustom.results_NN.size());
    ...
    public void goToNextActivity() {
        ...
        //pass original size image to getResultNdisplay.class
        //intent.putExtra("imagePath", String.valueOf(picturePath));
...
//        }
        ...
        intent.putExtra("SVM_Th", String.valueOf(SVM_Th));
...
//        Log.d(TAG, "ImgQ: " + ImgQ);
    ...
    public void runSVM(Mat mat, int num) {
    ...
    public void readSVMTextFile(int num) {
        ...
        for (int index = 0; index < 4; index++) {
            ...
            try {
                if (index == 0) {
                    InputStream svIS = getResources().openRawResource(getResources().getIdentifier(svStr, "raw", getPackageName()));
                    br = new BufferedReader(new InputStreamReader(svIS));
                } else if (index == 1) {
                    InputStream alphaIS = getResources().openRawResource(getResources().getIdentifier(alphaStr, "raw", getPackageName()));
                    br = new BufferedReader(new InputStreamReader(alphaIS));
                } else if (index == 2) {
                    InputStream sfIS = getResources().openRawResource(getResources().getIdentifier(sfStr, "raw", getPackageName()));
                    br = new BufferedReader(new InputStreamReader(sfIS));
                } else {
                    InputStream shiftIS = getResources().openRawResource(getResources().getIdentifier(shiftStr, "raw", getPackageName()));
